...
(\S~\ref{sec:related}). Appendix~\ref{sec:background} covers background
concepts such as \emph{linear logic} and \emph{sequent calculus}.
Appendix~\ref{sec:final_system} presents the final set of inference
rules, putting together everything seen in \S~\ref{sec:formal_system}.
...
satisfied by the program \mintinline{haskell}{(\x -> \y -> Nil)}. To synthesize
...
($\tensor$) pairs, \todo{qual a diferenca}
...
decomposing connectives. % (propositions we'll later try to invert,
...
% Esta parte em geral está a ficar pouco coerente, com a introdução do focusing,
  ...
  %
\todo[inline]{Context management -- To handle the context splitting
  required ... we augment the judgments above with .... where ....}
...
We begin with the right invertible rules:\todo{que decompoem a direita}
\begin{mathpar}
...
Followed by the left invertible rules, which ...\todo{fazem o que?}:
\[
  \begin{array}{c}
...
\qquad
  ...
  pattern. \todo{The focus right rules \dots }
...
functions to synthesize any existential type if, altogether, we've already used
...
``existential synthesis''. \todo{alinhar mais com a premissa
  respectiva da regra}
%
...
List a)}, we wouldn't try to synthesize \mintinline{haskell}{(a * List a)},
...
The synthesis module iterates over the synthesis marks and runs the synthesizer
