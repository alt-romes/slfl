...
\todo[inline]{Tens de explicar o que significa o $\bang$, see above.}
...
\todo[inline]{Tens de dizer o que significa o refinamento concreto e o
que e a funcao mult e add}
...
augmented with three keywords: ``using'', ``depth'' and
``assert''. Through them, we can fine-tune the synthesis process and,
respectively, force certain functions to be present in the synthesis
outcome, allow a \emph{deeper} search in the valid programs space, and
...
\todo[inline]{Indicar o judgment ``declarativo'' e depois os
  judgments em focusing}
...
are not invertible. Asynchronous connectives are dual. \todo{Say which
  are which}
%
...
\todo[inline]{Isto so faz sentido olhando para os judgments}
...
\todo[inline]{A frente de cada ``descricao'', escreve o tipo pois
  ajuda o leitor}
...
first rule \todo{qual ``first''? referencia por nome} is read: to synthesize a program of type $A \lolli B$ while inverting
...
\todo[inline]{Aqui tens de explicar uma regra pelo menos}
...
recursive and $\Rho'_C = \Rho_C$ otherwise (similarly for $\Rho'_D$):
%
\begin{mathpar}
    \infer*[right=(adtR)]
...
\todo{consume? deconstruct?} all constructor parameters, and since we
...
non-determinism and infinite recursion.\todo{check} 
%
...
branch. % (TODO: rewrite? O bold é meio estranho não?)
...
uses the result as the key\todo{via a hashconsing library?}.
%
...
>>>>>>> 4290cfe31b8de10ca70f772fbc7d5fdc9542273c
%
