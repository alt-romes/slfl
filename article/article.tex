...
With this section we aim to formalize the implicit \todo{mais uma vez\dots} rules that guide
the synthesis of more \todo{nao se faz sintese de specs, mas sim a
  partir de specs} expressive specifications, alongside the already
explicit rules that model the core of the synth \todo{synthesis} process, resulting in
a \emph{sound} set of rules that hopes to serve as a source of ideas
\todo{ nao me parece\dots As regras caracterizam o que esta
  implementado, e esse o objectivo, nao e ser uma fonte de ideias\dots }
on taming synthesis of recursive functions with richer types in a
linear context, as well as describe the system in enough detail for
...
explained in\ref{sec:background} \todo{se o background nao vai ser
  incluido no documento principal, nao deves referenciar assim tao
  directamente para o conteudo principal.}, focusing separates the proof search into two
...
\todo{a sintaxe $\lambda x \rightarrow M$ nao e mt standard, usa
  $\lambda x . M$}
...
asynchronous. If we're left focused on an atomic proposition we either
instance \todo{instantiate}
...
\todo[inline]{Não uses Foo e Bar. $T_1$, $T_2$ , $\dots$ para
  tipos. $C_1$, $C_2$, $\dots$ para os construtores.}
...
be the type the constructor constructs \todo{ Na escrita, repetition
  does not legitimize. Constructors can take as arguments values of
  the type they are defining, ou algo assim}. Seemingly small, this change has a big
...
derivation for the synth goal $\te{Foo}$ is also infinite.
\begin{mathpar}
    \infer*[left=(adtR)]
    {
        \infer*[left=(adtR)]
...
proof of a type to construct it. \todo{Não se percebe muito bem a
  segunda parte da frase, rephrase!}
%
%
For this, we need two more contexts, $\Rho_C$
for restrictions \todo{constraints} on construction and $\Rho_D$ for restrictions on
...
\end{mathpar}
These modifications prevent the derivation from not ending
\todo{infinite derivations} for the scenarios described above,
however, they also greatly limit the space of derivable programs,
leaving the synthetiser effectively unable to synth specifications
with recursive types\todo{refrasear tendo em conta que se sintetiza a
  partir de specs e nao specs}, which is so undesirable that we add three rules
to complement the restrictions on construction and destruction of
recursive types.
...
\todo[inline]{esta frase nao me parece muito necessaria}
...
solutions.  Instead, our framework only allows the use of recursion after having
...
the \emph{Cons} branch. % (TODO: rewrite)


% TODO: E como evitamos coisas trivialmente estúpidas?


\todo[inline]{Polymorphic}

\mypara{Polymorphic Types} A polymorphic specification is a type $X$ \todo{T} and a set of
bound variables that quantify universally the type variables in $X$, called a
\emph{scheme} ($\forall \overline{\alpha}.\ \tau$).
\todo[inline]{Não é um tipo e um conjunto de variáveis bound. É um
  tipo da forma $\forall \overline{\alpha}.\ T$ em que $\dots$ }
Synthesis of a scheme is straightforward. \todo{Diria que nao e
  especialmente straightforward.}
First, \textbf{type variables are considered atomic types}, then, instantiate
the bound vars\todo{variables} of the scheme as described by the Hindley-Milner inference
...
\lolli \alpha$ could be instanced \todo{instantiated} to
...
polymorphic * TODO: rewrite this?.\todo[inline]{Use an example.}
...
all bound variables. \todo{Explica o que e isto} This way inference can be done without worrying about name
conflicts.
...
hashes the relevant values and uses the result as the key. Using LogicT, a
branch we want to memoize might fail -- it's desireable to also record that a
synthesis fails, as it happens more than successful results. We capture failure
...
of memoization on the performance. It's more noticeable in synthesis with
...
\begin{center}
    \begin{tabular}{ |c|c|c|c|c| }
        ...
        \hline
        insertWithKey & 13.31 s ± 593.4 ms & 2.667 s ± 284.3 ms
    \end{tabular}
\end{center}
