...
variables $x$. For instance, $\lambda x{:}A.x$, the identity function, is a functional abstraction
...
as types for the simply-typed $\lambda$-calculus (``propositions as types'' observation), their proofs are
exactly the program with that type (``proofs as programs'' observation), and checking a proof is type checking a program (``proof checking as type checking'' observation)~\cite{}.
...
formalized. The following rule captures the nature of a
...
To present the formalization of this logic, besides the new connectives, we 
need to introduce the \emph{resource-aware context} $\Delta$.
 In contrast to the previously seen $\Gamma$, $\Delta$ is also a list of variables and their types,
 but in which each and every variable must be used exactly once.
To introduce the connective $\tensor$ which defines a multiplicative pair of propositions,
 we must use exactly all the resources ($\Delta_1, \Delta_2$) needed
...
it useable more than once. To distinguish the ``infinite'' variables, a separate context is used --
$\Gamma$. $\Gamma$ holds the ``infinite'' resources, and $\Delta$ the resources that can only be used once.
