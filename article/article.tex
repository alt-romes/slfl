...
synthesis from more expressive specifications, alongside the already
well defined rules that model the core of the synthesis process, resulting in
...
\[
    \textrm{data}\ T = C_1\ X_1\ \vert\ C_2\ X_2\ \vert\
\dots\ \vert\ C_n\ X_n
\]
...
be the type the constructor constructs \todo{ Na escrita, repetition does not
legitimize. Constructors can take as arguments values of the type they are
defining, ou algo assim}. Seemingly small, this change has a big impact in the
synthesis process. Take, for instance, the ADT defined as $\textrm{data}\ T =
C_1\ T$, the synth goal $T \lolli C$, and its derivation:
%
\begin{mathpar}
    \infer*[right=($\lolli R$)]
    {
...
proofs of its branches, and, simetrically, don't allow construction of an
ADT when trying to synth an argument for its constructor. \todo{R: A segunda parte confusa era da frase de
  cima?, está melhor?}
%
%
...
the synthetiser effectively unable to synth from specifications with recursive
types\todo{refrasear tendo em conta que se sintetiza a partir de specs e nao
specs R: refrasear mas só com "from" não fica bem?}, which is so undesirable that we add three rules to complement the
...
then treating its type variables agnostically. \todo{R: Melhor que
straightforward?}
