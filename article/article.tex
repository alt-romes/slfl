...
consisting of \emph{bottom-up} proof search in linear logic augmented with
resource management~\cite{} and focusing~\cite{} as techniques to, respectively,
solve resource related non-determinism, and eliminate from the proof-search
don't-care non-determinism. A more in depth explanation is in
section~\ref{sec:formal_system}.

With the core in place, we can synthetise programs from specifications that use
...
All additions to the synthesis are extensions of the proof-search in linear
logic system -- in the end, the process still consists of constructing a proof.
Because we can only construct valid proofs (a wrong proof isn't a proof), we
know that all produced synthesis results are correct: \textbf{all programs are
correct by construction}, i.e. if we're able to synthetise it, then it typechecks,
and therefore fullfills the specification.
