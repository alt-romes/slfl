...
%
% TODO: Make it actually output this... need pattern matching on lambdas + minimization...
\begin{minted}{haskell}
...
(inspired by~\cite{FPnotes...}) -- in which the
...
its constructors stand for any ADT defined as \mintinline{haskell}{data T = C1
X1 | C2 X2 | ... | Cn Xn}:
%
\begin{mathpar}
    \infer*[right=(adtR)]
...
The benchmarks\footnote{Considering the full pipeline pass; 2017 MBP; 8GB RAM;
2.3Ghz Intel Core i5; with benchmarking library Criterion} are separated into groups: theorems of linear logic, recursive
...
% TODO Verificar nomes dos linear logic theoresm...
{\scriptsize
\begin{center}
    \begin{tabular}{ |c|c|c|c|c| }
        ...
        % & mapKeys & 1s & yes & yes \\
        \hline
        \multirow{1}{4em}{Misc} & either & $197\mu s\pm 5.3\mu s$ && \\ 
        \hline
        \multirow{2}{4em}{Array} & & & \emph{depth 3} & \emph{freeze, foldl} \\
        & array & $80ms\pm 870\mu s$ & \emph{using (foldl),depth 3} & \emph{newMArray,write} \\
        \hline
    \end{tabular}
  \end{center}
}
