{-# LANGUAGE LambdaCase #-}
module Program (Program(..), ADTD(..), trivialProgram, makeFrontendMarksCtx) where
...
-- | Copy the Marks info from the core bindings to the general program data
makeFrontendMarksCtx :: Program -> Program
makeFrontendMarksCtx (Program as bs ts cs) = undefined

-- type MarksTypes = Map.Map Int ([(String, Scheme)], Maybe Type)

-- copyMarksTypesModule :: Program -> Program -> Program
-- copyMarksTypesModule (Program adts bs _ _) (Program _  _  _ cbs) = Program adts $ zipWith copyMarksTypes bs cbs
--     where
